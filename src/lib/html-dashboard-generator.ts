...
export function gerarHTMLDashboard(dados: DadosRelatorio): string {
  const { receitaTotal, despesaTotal, saldo, receitasPorCategoria, despesasPorCategoria, periodo, titulo } = dados;
  const tituloFinal = titulo || 'Resultados Financeiros';
...
export function gerarHTMLPizzaReceitas(dados: DadosRelatorio): string {
  const { receitaTotal, despesaTotal, saldo, receitasPorCategoria, periodo, titulo } = dados;
  const tituloFinal = titulo || 'Resultados Financeiros';
...
export function gerarHTMLPizzaDespesas(dados: DadosRelatorio): string {
  const { receitaTotal, despesaTotal, saldo, despesasPorCategoria, periodo, titulo } = dados;
  const tituloFinal = titulo || 'Resultados Financeiros';
...
export function gerarHTMLDuasPizzas(dados: DadosRelatorio): string {
  const { receitaTotal, despesaTotal, saldo, receitasPorCategoria, despesasPorCategoria, periodo, titulo } = dados;
  const tituloFinal = titulo || 'Resultados Financeiros';
...
export function gerarHTMLBarras(dados: DadosRelatorio, tipo: 'receita' | 'despesa'): string {
  const { receitaTotal, despesaTotal, saldo, receitasPorCategoria, despesasPorCategoria, periodo, titulo } = dados;
  const tituloFinal = titulo || 'Resultados Financeiros';
  ...
  const dadosGrafico = tipo === 'receita' ? receitasPorCategoria : despesasPorCategoria;
  const tituloGrafico = tipo === 'receita' ? 'Receitas por Categoria' : 'Despesas por Categoria';
...
<body>
  <div class="dashboard">
    ...
    <div class="chart-container">
      <div class="chart-title">${tituloGrafico}</div>
      <div class="chart-wrapper">
  ...
  <script>
    new Chart(document.getElementById('barrasChart'), {
      type: 'bar',
      data: {
        labels: ${JSON.stringify(dadosGrafico.labels)},
        datasets: [{
          label: '${tipo === 'receita' ? 'Receitas' : 'Despesas'}',
          data: ${JSON.stringify(dadosGrafico.values)},
          backgroundColor: '${cor}',
          borderRadius: 8,
          borderWidth: 0
        }]
...
export function gerarHTMLComparativo(dados: DadosRelatorio): string {
  const { receitaTotal, despesaTotal, saldo, periodo, titulo } = dados;
  const tituloFinal = titulo || 'Resultados Financeiros';
